              ...
              $mdDialog.show({
                ...
                .then(function (v) {
                      ...
                      if (typeof scope.currentDate !== 'undefined') {
                          var m = moment(scope.currentDate);
                          value = m.isValid() ? m.format(scope.format) : ''
                      }

                      if (scope.$parent.dialog){
                          if (scope.$parent.pedido){
                              if (element[0].attributes[0].nodeValue == "pedido.horaCarga"){
                                  scope.$parent.pedido.horaCarga = value;
                              }
                              if (element[0].attributes[0].nodeValue == "pedido.horaEntrega"){
                                  scope.$parent.pedido.horaEntrega = value;
                              }
                              scope.$parent.dialog(null,scope.$parent.pedido);
                          } else {
                              scope.$parent.dialog(null,undefined);
                          }
                      }
                }, function () {
                  isOn = false;
